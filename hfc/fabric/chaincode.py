...
#         print(f"res is ------->> {res}")
    ...
    # async def createasset(self, requestor, channel_name, peers, args,
    #                 cc_type=CC_TYPE_GOLANG, fcn='CreateAsset', transient_map=None):
    #     """
    #     Query chaincode
    #
    #     :param requestor: User role who issue the request
    ...
    #     #
    #     res = await asyncio.gather(*responses)
    #     # print(f"responses: {res}, proposal:{proposal}, header:{header}")
    #     tran_req = utils.build_tx_req((res, proposal, header))
    #     # print(f"tran_req --------->>{tran_req}")
    #     # for x in tran_req.responses:
    #     #     print("tran_req is: ",x)
    #     #     print(x.response.payload.decode('utf-8'))
    #
    #     if not all([x.response.status == SUCCESS_STATUS for x in tran_req.responses]):
    #         raise ChaincodeExecutionError(res)
    #     cc_version = "v1.1"
    ...
    async def createasset(self, requestor, channel_name, peers, args,
                     cc_type=CC_TYPE_GOLANG,
                     fcn='CreateAsset', cc_pattern=None,
                     transient_map=None,
                     wait_for_event=False,
                     wait_for_event_timeout=DEFAULT_WAIT_FOR_EVENT_TIMEOUT,
                     grpc_broker_unavailable_retry=0,
